...
export function generateConsensusMarkdown(debate: ExportableDebate): string {
  ...
  let md = `# Iris AI Council — Final Consensus Resolution\n\n`;
  md += `> **Status:** Ratified Dialectic Synthesis\n`;
  md += `> **Inquiry:** ${debate.prompt}\n`;
  md += `> **Date:** ${humanDate} (${dateStr})\n`;
  md += `> **Protocol:** ${debate.protocol.toUpperCase()} Consensus Architecture\n\n`;

  if (debate.metrics) {
    md += `### Consensus Telemetry\n\n`;
    md += `| Metric | Value | Description |\n`;
    md += `| :--- | :--- | :--- |\n`;
    md += `| **Consensus Score** | **${debate.metrics.consensusRate ?? 90}%** | Degree of cross-agent alignment |\n`;
    md += `| **Contention Index** | **${debate.metrics.contentionLevel ?? 'Moderate'}** | Adversarial scrutiny intensity |\n`;
    if (debate.metrics.resolvedPointsCount) {
      md += `| **Flaws Fortified** | **${debate.metrics.resolvedPointsCount}** | Edge-case vulnerabilities neutralized |\n`;
    }
    if (debate.metrics.durationMs) {
      md += `| **Deliberation Time** | **${(debate.metrics.durationMs / 1000).toFixed(2)}s** | Multi-round convergence latency |\n`;
    }
    md += `\n---\n\n`;
  }

  md += `## Ratified Synthesis & Executive Consensus\n\n`;
  md += `${(debate.finalOutput || 'No consensus output recorded.').trim()}\n\n`;
  md += `---\n\n`;
  md += `*Formulated and ratified by the Iris AI Multi-Agent Consensus Council (Architect • Skeptic • Arbiter).*\n`;
...
export function generateMarkdownTranscript(debate: ExportableDebate): string {
  ...
  let md = `# Deliberation Council Transcript\n\n`;
  md += `**Inquiry:** ${debate.prompt}\n`;
  ...
  if (debate.metrics) {
    md += `\n### Consensus Telemetry\n`;
    md += `- **Consensus Score:** ${debate.metrics.consensusRate ?? 90}%\n`;
    md += `- **Contention Level:** ${debate.metrics.contentionLevel ?? 'Moderate'}\n`;
    if (debate.metrics.resolvedPointsCount) {
      md += `- **Resolved Points:** ${debate.metrics.resolvedPointsCount} fatal flaws mitigated\n`;
    }
    if (debate.metrics.durationMs) {
      md += `- **Deliberation Duration:** ${(debate.metrics.durationMs / 1000).toFixed(2)}s\n`;
    }
  }

  md += `\n---\n\n## Dialectic Proceedings\n\n`;
  ...
  if (debate.finalOutput) {
    md += `## Final Council Consensus Resolution\n\n`;
    md += `${debate.finalOutput.trim()}\n\n`;
    md += `---\n*Generated by Iris AI Multi-Agent Consensus Council*\n`;
  }
...
export function generatePlainTextTranscript(debate: ExportableDebate): string {
  ...
  let txt = `${divider}\n`;
  txt += `IRIS AI DELIBERATION COUNCIL - DEBATE TRANSCRIPT\n`;
  txt += `${divider}\n\n`;
  ...
  if (debate.metrics) {
    txt += `\nCONSENSUS METRICS:\n`;
    txt += `  - Consensus Score: ${debate.metrics.consensusRate ?? 90}%\n`;
    txt += `  - Contention: ${debate.metrics.contentionLevel ?? 'Moderate'}\n`;
    if (debate.metrics.resolvedPointsCount) {
      txt += `  - Flaws Resolved: ${debate.metrics.resolvedPointsCount}\n`;
    }
  }
  ...
  if (debate.finalOutput) {
    txt += `${divider}\nFINAL COUNCIL CONSENSUS RESOLUTION\n${divider}\n\n`;
    txt += `${debate.finalOutput.trim()}\n\n`;
  }

  txt += `${divider}\nEnd of Deliberation Transcript\n`;
